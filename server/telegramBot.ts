...
export function initTelegramBot(token: string, baseUrl: string): TelegramBot {
  ...
  bot.on("callback_query", async (query) => {
    ...
    if (data.startsWith("cashout_deny_")) {
      ...
      if (adminData && adminData.cashoutId === cashoutId) {
        ...
        await bot.editMessageText(deniedMsg, { chat_id: originalChatId, message_id: query.message?.message_id! }).catch(() => {});
        try { await bot.sendMessage(REPORT_GROUP_ID, `❌ **CASHOUT DENIED**\n\n👤 Employee: ${coState.employeeName}\n🎮 Game: ${coState.game}\n💰 Amount: $${coState.amount}\n👨‍⚖️ Denied By: ${denierName} (Admin)\n🆔 Cashout ID: ${cashoutId}`); } catch (_) {}
    ...
    if (data.startsWith("cashout_approve_")) {
      ...
      if (adminData && adminData.cashoutId === cashoutId) {
        ...
        await bot.editMessageText(approvedMsg, { chat_id: originalChatId, message_id: query.message?.message_id! }).catch(() => {});
        const reportMsg = `✅ **CASHOUT APPROVED & RECORDED**\n\n📊 CASHOUT SUMMARY\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n🎮 Game: ${coState.game}\n🎯 Points Redeemed: ${coState.points}\n🎫 Playback Points: ${coState.playback_points}\n💵 Tip: $${coState.tip}\n💰 Final Cashout: $${coState.amount}\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n👤 Employee: ${coState.employeeName}\n👨‍⚖️ Approved By: ${approverName} (Admin)\n🆔 Cashout ID: ${cashoutId}`;
        try {
          await bot.sendMessage(REPORT_GROUP_ID, reportMsg);
          if (coState.mediaType === "photo" && coState.photoFileId) { await bot.sendPhoto(REPORT_GROUP_ID, coState.photoFileId, { caption: `📸 Payment Method Screenshot\n${coState.mediaCaption}` }); }
          else if (coState.mediaType === "text" && coState.mediaCaption) { await bot.sendMessage(REPORT_GROUP_ID, `📝 Payment Details:\n${coState.mediaCaption}`); }
        } catch (_) {}
        adminMessages.delete(query.message?.message_id!);
        await disableUserControlMsg(cashoutId);
      }
    ...
    if (data.startsWith("pending_edit_")) {
      ...
      const pending = pendingCashouts.get(cashoutId);
      if (!pending) { await bot.answerCallbackQuery(query.id, { text: "❌ This cashout is no longer editable.", show_alert: true }); return; }
    ...
    if (data.startsWith("pending_cancel_")) {
      ...
      const pending = pendingCashouts.get(cashoutId);
      if (!pending) { await bot.answerCallbackQuery(query.id, { text: "❌ This cashout is no longer active.", show_alert: true }); return; }
      const cancelerName = query.from?.first_name || query.from?.username || "Unknown";
      removeCashoutRecord(cashoutId);
      try { await bot.editMessageText(`🚫 **CASHOUT CANCELLED** by ${cancelerName}\n\n🆔 Cashout ID: ${cashoutId}`, { chat_id: pending.adminChatId, message_id: pending.adminMsgId }); } catch (_) {}
      try { await bot.sendMessage(REPORT_GROUP_ID, `🚫 **CASHOUT CANCELLED**\n\n👤 Employee: ${pending.state.employeeName}\n💰 Amount: $${pending.state.amount}\n🙋 Cancelled By: ${cancelerName}\n🆔 Cashout ID: ${cashoutId}`); } catch (_) {}
      adminMessages.delete(pending.adminMsgId);
      pendingCashouts.delete(cashoutId);
      userState.delete(chatId);
      try { await bot.editMessageText(`🚫 **Cashout Cancelled**\n\nYour cashout (ID: ${cashoutId}) has been cancelled successfully.`, { chat_id: chatId, message_id: query.message?.message_id! }); } catch (_) {}
      await bot.answerCallbackQuery(query.id, { text: "🚫 Cashout cancelled!", show_alert: true });
      return;
    }

    // ─── CASHOUT NUMBER KEYBOARD HANDLER ──────────────────────────────────────
    if (data.startsWith("cashout_num_")) {
      if (!state || state.type !== "cashout") { await bot.answerCallbackQuery(query.id); return; }
      const action = data.replace("cashout_num_", "");
      if (action === "back") { state.amountInput = (state.amountInput || "").slice(0, -1); }
      else if (action === "dot") { if (!(state.amountInput || "").includes(".")) state.amountInput = (state.amountInput || "") + "."; }
      else if (action === "done") {
        const value = parseFloat(state.amountInput || "0");
        if (state.step === "cashout_points") {
          state.points = value; state.step = "cashout_playback"; state.amountInput = "0";
          await bot.sendMessage(chatId, `✅ Points: ${value}\n\nStep 3: Enter Playback Points (0 if none):`, cashoutNumberKeyboard);
        } else if (state.step === "cashout_playback") {
          state.playback_points = value.toString(); state.step = "cashout_tip"; state.amountInput = "0";
          await bot.sendMessage(chatId, `✅ Playback: ${value}\n\nStep 4: Enter Tip Amount (0 if none):`, cashoutNumberKeyboard);
        } else if (state.step === "cashout_tip") {
          state.tip = value; state.step = "cashout_amount"; state.amountInput = "";
          await bot.sendMessage(chatId, `✅ Tip: $${value}\n\nStep 5: Enter Final Cashout Amount:`, cashoutNumberKeyboard);
        } else if (state.step === "cashout_amount") {
          if (isNaN(value) || value <= 0) { await bot.answerCallbackQuery(query.id, { text: "Please enter a valid amount", show_alert: true }); return; }
          state.amount = value; state.step = "cashout_review";
    ...
    // ─── INCOME NUMBER KEYBOARD HANDLER ───────────────────────────────────────
    if (data.startsWith("num_")) {
      if (!state || state.type !== "income") { await bot.answerCallbackQuery(query.id); return; }
      ...
      if (action === "back") { state.amountInput = (state.amountInput || "").slice(0, -1); }
        ...
        if (state.step === "amount") {
          state.amount = value; state.step = "game";
          await bot.sendMessage(chatId, `💰 Amount: $${value}\n\nSelect Game(s):`, { reply_markup: { inline_keyboard: [...gameKeyboard.reply_markup.inline_keyboard, [{ text: "✅ Done", callback_data: "game_done" }]] } });
        } else if (state.step === "points") {
          state.points = value;
          const cst = getCST(); const gameStr = state.selectedGames.join(", ");
          const row = `${cst.date},${cst.time},${cst.day},"${state.groupName}","${state.employeeName}",${state.amount},"${gameStr}",${value},\n`;
    ...
    if (data.startsWith("game_")) {
      if (!state) { await bot.answerCallbackQuery(query.id); return; }
      if (data === "game_done") {
        if (state.type === "income") {
          if (state.selectedGames.length === 0) { await bot.answerCallbackQuery(query.id, { text: "Please select at least one game", show_alert: true }); return; }
          state.step = "points"; state.amountInput = "";
          await bot.sendMessage(chatId, `🎮 Games: ${state.selectedGames.join(", ")}\n\nEnter Points:`, numberKeyboard);
        }
      } else if (data === "game_Other") {
        if (state.type === "income") state.step = "custom_game"; else state.step = "cashout_custom_game";
        ...
        if (state.type === "income") {
          ...
          await bot.sendMessage(chatId, `✅ Selected: ${state.selectedGames.join(", ")}`, { reply_markup: { inline_keyboard: [...gameKeyboard.reply_markup.inline_keyboard, [{ text: "✅ Done", callback_data: "game_done" }]] } });
        } else {
    ...
    // ─── CASHOUT CONFIRM & EDIT ───────────────────────────────────────────────
    if (data === "cashout_confirm") {
      if (!state || state.type !== "cashout") { await bot.answerCallbackQuery(query.id); return; }
      state.step = "cashout_pending_admin"; state.updatedAt = getCST().isoTime;
      ...
      const adminMsgObj = await bot.sendMessage(chatId, adminMsg, { reply_markup: { inline_keyboard: [[{ text: "✅ APPROVE", callback_data: `cashout_approve_${state.cashoutId}` }, { text: "❌ DENY", callback_data: `cashout_deny_${state.cashoutId}` }]] } }).catch(() => null);
      if (adminMsgObj) {
        adminMessages.set(adminMsgObj.message_id, { cashoutId: state.cashoutId, state: { ...state }, chatId });
        const userControlMsg = await bot.sendMessage(chatId, `✅ **Cashout Submitted!**\n\nYour cashout is pending admin approval.\n\n🆔 Cashout ID: ${state.cashoutId}\n💰 Amount: $${state.amount}\n🎮 Game: ${state.game}\n\nYou can still edit or cancel until admin acts:`, buildUserControlKeyboard(state.cashoutId)).catch(() => null);
    ...
    if (data.startsWith("cashout_edit_")) {
      ...
      else if (field === "playback") { state.step = "cashout_playback"; state.amountInput = state.playback_points; await bot.sendMessage(chatId, `🎫 Current Playback Points: ${state.playback_points}\n\nEdit Playback Points Value:`, cashoutNumberKeyboard); }
    ...
    if (data === "cashout_picture") { if (state) { state.step = "waiting_picture"; await bot.sendMessage(chatId, "📸 Please send the payment method screenshot:"); } await bot.answerCallbackQuery(query.id); return; }
    if (data === "cashout_text") { if (state) { state.step = "waiting_text"; await bot.sendMessage(chatId, "📝 Please type the payment details:"); } await bot.answerCallbackQuery(query.id); return; }
  ...
  bot.on("text", async (msg) => {
    ...
    if (state) {
      if (state.type === "income" && state.step === "custom_game") {
        ...
        await bot.sendMessage(chatId, `Added "${msg.text}"\nSelected: ${state.selectedGames.join(", ")}`, { reply_markup: { inline_keyboard: [...gameKeyboard.reply_markup.inline_keyboard, [{ text: "✅ Done", callback_data: "game_done" }]] } });
      }
      if (state.type === "cashout" && state.step === "waiting_text") {
        state.mediaCaption = msg.text; state.mediaType = "text"; state.step = "cashout_game"; state.amountInput = "";
  ...
  bot.onText(/\/delete/, async (msg) => {
    if (!msg.reply_to_message) { await bot.sendMessage(msg.chat.id, "❌ Please **reply** to the message you want to delete with /delete"); return; }
    const chatId = msg.chat.id; const repliedToMsgId = msg.reply_to_message.message_id;
    if (chatId === CASHOUT_GROUP_ID) {
      ...
      if (idMatch) {
        const cashoutId = idMatch[0]; removeCashoutRecord(cashoutId);
        ...
        try { await bot.sendMessage(REPORT_GROUP_ID, `🗑️ **CASHOUT DELETED**\n\nCashout ID: ${cashoutId}\nDeleted by: ${deleterName}`); } catch (_) {}
        return;
      }
      await bot.sendMessage(chatId, "❌ Could not identify the cashout from the replied message."); return;
    }
    if (!fs.existsSync(RECORDS_FILE)) { await bot.sendMessage(chatId, "No records found."); return; }
    const lines = fs.readFileSync(RECORDS_FILE, "utf-8").trim().split("\n");
    if (lines.length <= 1) { await bot.sendMessage(chatId, "No records to delete."); return; }
    const cst = getCST(); const lastLine = lines[lines.length - 1];
    ...
    try { await bot.sendMessage(REPORT_GROUP_ID, `🗑️ Deletion recorded for group: ${parts[3] || 'Unknown'}`); } catch (_) {}
  });
  ...
  bot.setWebHook(baseUrl + webhookPath).then(() => console.log("✅ Webhook set successfully")).catch(err => console.error("Webhook failed:", err));
  return bot;
}
...
function showCashoutReview(chatId: number, state: any, bot: TelegramBot) {
  ...
  bot.sendMessage(chatId, reviewText, { reply_markup: { inline_keyboard: [[{ text: "✏️ Edit Game", callback_data: "cashout_edit_game" }], [{ text: "✏️ Edit Points", callback_data: "cashout_edit_points" }], [{ text: "✏️ Edit Playback Points", callback_data: "cashout_edit_playback" }], [{ text: "✏️ Edit Tip", callback_data: "cashout_edit_tip" }], [{ text: "✏️ Edit Amount", callback_data: "cashout_edit_amount" }], [{ text: "✅ Confirm & Submit", callback_data: "cashout_confirm" }]] } }).catch(() => {});
}
